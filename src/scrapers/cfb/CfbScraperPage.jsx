...
import { syncCfbScoreboard, syncCfbScoreboardRange, todayYYYYMMDD } from '../../services/espnCfbScoreboard'
...
export default function CfbScraperPage() {
  ...
  const [storedTeams, setStoredTeams] = useState(() => getCfbTeamsD1())
  const recentMasterChanges = getMasterChangeHistory({ sport: 'CFB', limit: 100 })

  const refreshStored = () => {
    setStoredTeams(getCfbTeamsD1())
  }
  ...
  const handleImportFromEspn = async () => {
    ...
    try {
      const result = start === end
        ? await syncCfbScoreboard(start, { skipSave: true, skipMerge: true })
        : await syncCfbScoreboardRange(start, end, { skipSave: true, skipMerge: true })
  ...
  const handleSyncWithMaster = () => {
    ...
    setError(null)
    const detailed = compareWithMasterDetailed(games, 'CFB')
    ...
    mergeGamesIntoMaster(games, 'CFB', runFetchedAtIso || new Date().toISOString())
  ...
  const confirmSyncWithMaster = () => {
    ...
    mergeGamesIntoMaster(syncPreview.games, 'CFB', syncPreview.fetchedAtIso)
    setMergeReport({
  ...
  const handleExportCsv = () => {
    ...
    a.href = url
    a.download = `CFB-schedule-${todayYYYYMMDD()}.csv`
    a.click()
          ...
          <div className={styles.controls}>
            ...
            <div className={styles.actions}>
              <button
                ...
                aria-label="Import CFB schedule from ESPN"
              >
          ...
          >
            ...
            <div className={styles.modalBody}>
              {recentMasterChanges.length === 0 ? (
                <p className={styles.compareSummary}>No recorded CFB master changes yet.</p>
              ) : (
                <ul className={styles.syncChangesList}>
              ...
              <tbody>
                {games.map((game) => {
                  ...
                  return (
                    ...
                    >
                      <td>
                        <span className={styles.sportTag}>{game.sport || 'CFB'}</span>
                      </td>
                      <td>{times.et}</td>
                      <td>{times.dub}</td>
                      <td>{times.melb}</td>
