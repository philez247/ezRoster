...
import {
  syncNflScoreboard,
  syncNflScoreboardRange,
  todayYYYYMMDD,
} from '../../services/espnNfl'
...
import styles from './NflScraper.module.css'
...
export default function NflScraperPage() {
  ...
  const recentMasterChanges = getMasterChangeHistory({ sport: 'NFL', limit: 100 })
  ...
  const handleImportFromEspn = async () => {
    ...
    try {
      ...
      let result
      if (start === end) {
        result = await syncNflScoreboard(start, onProgress, opts)
      } else {
        result = await syncNflScoreboardRange(start, end, onProgress, opts)
      }
  ...
  const handleSyncWithMaster = () => {
    ...
    setError(null)
    const detailed = compareWithMasterDetailed(games, 'NFL')
    ...
    mergeGamesIntoMaster(games, 'NFL', runFetchedAtIso || new Date().toISOString())
  ...
  const confirmSyncWithMaster = () => {
    ...
    mergeGamesIntoMaster(syncPreview.games, 'NFL', syncPreview.fetchedAtIso)
    setMergeReport({
  ...
  const handleExportCsv = () => {
    ...
    a.href = url
    a.download = `nfl-schedule-${todayYYYYMMDD()}.csv`
    a.click()
          ...
          <div className={styles.controls}>
            ...
            <div className={styles.actions}>
              <button
                ...
                aria-label="Import NFL schedule from ESPN"
              >
          ...
          >
            ...
            <div className={styles.modalBody}>
              {recentMasterChanges.length === 0 ? (
                <p className={styles.compareSummary}>No recorded NFL master changes yet.</p>
              ) : (
                <ul className={styles.syncChangesList}>
              ...
              <tbody>
                {games.map((game) => {
                  ...
                  return (
                    <tr key={game.gameId}>
                      <td>
                        <span className={styles.sportTag}>{game.sport || 'NFL'}</span>
                      </td>
                      <td>{times.et}</td>
                      <td>{times.dub}</td>
                      <td>{times.melb}</td>
        ...
        <div className={styles.mobileResults}>
          {games.length ? (
            ...
            <p className={styles.empty}>
              No games loaded. Select a date range and click Import NFL.
            </p>
          )}
        </div>
        {games.length === 0 && (
