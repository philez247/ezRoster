...
import {
  syncNhlScoreboard,
  syncNhlScoreboardRange,
  todayYYYYMMDD,
} from '../../services/espnNhl'
...
import styles from './NhlScraper.module.css'
...
export default function NhlScraperPage() {
  ...
  const recentMasterChanges = getMasterChangeHistory({ sport: 'NHL', limit: 100 })
  ...
  const handleImportFromEspn = async () => {
    ...
    try {
      ...
      const opts = { skipMerge: true }
      let result
      if (start === end) {
        result = await syncNhlScoreboard(start, onProgress, opts)
      } else {
        result = await syncNhlScoreboardRange(start, end, onProgress, opts)
      }
  ...
  const handleSyncWithMaster = () => {
    ...
    setError(null)
    const detailed = compareWithMasterDetailed(games, 'NHL')
    ...
    mergeGamesIntoMaster(games, 'NHL', runFetchedAtIso || new Date().toISOString())
  ...
  const confirmSyncWithMaster = () => {
    ...
    mergeGamesIntoMaster(syncPreview.games, 'NHL', syncPreview.fetchedAtIso)
    setMergeReport({
  ...
  const handleExportCsv = () => {
    ...
    a.href = url
    a.download = `nhl-schedule-${todayYYYYMMDD()}.csv`
    a.click()
          ...
          <div className={styles.controls}>
            ...
            <div className={styles.actions}>
              <button
                ...
                aria-label="Import NHL schedule from ESPN"
              >
          ...
          >
            ...
            <div className={styles.modalBody}>
              {recentMasterChanges.length === 0 ? (
                <p className={styles.compareSummary}>No recorded NHL master changes yet.</p>
              ) : (
                <ul className={styles.syncChangesList}>
              ...
              <tbody>
                {games.map((game) => {
                  ...
                  return (
                    <tr key={game.gameId}>
                      <td>
                        <span className={styles.sportTag}>{game.sport || 'NHL'}</span>
                      </td>
                      <td>{times.et}</td>
                      <td>{times.dub}</td>
                      <td>{times.melb}</td>
        ...
        <div className={styles.mobileResults}>
          {games.length ? (
            ...
            <p className={styles.empty}>
              No games loaded. Select a date range and click Import NHL.
            </p>
          )}
        </div>
        {games.length === 0 && (
