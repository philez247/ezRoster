...
export default function MlbScraperPage() {
  ...
  const handleImportFromEspn = async () => {
    ...
    try {
      ...
      let result
      if (start === end) {
        result = await syncMlbScoreboard(start, onProgress, opts)
      } else {
        result = await syncMlbScoreboardRange(start, end, onProgress, opts)
      }
  ...
  const handleSyncWithMaster = () => {
    ...
    setError(null)
    const detailed = compareWithMasterDetailed(games, 'MLB')
    ...
    mergeGamesIntoMaster(games, 'MLB', runFetchedAtIso || new Date().toISOString())
  ...
  const confirmSyncWithMaster = () => {
    ...
    mergeGamesIntoMaster(syncPreview.games, 'MLB', syncPreview.fetchedAtIso)
    setMergeReport({
  ...
  const handleExportCsv = () => {
    ...
    a.href = url
    a.download = `mlb-schedule-${todayYYYYMMDD()}.csv`
    a.click()
          ...
          <div className={styles.controls}>
            ...
            <div className={styles.actions}>
              <button
                ...
                aria-label="Import MLB schedule from ESPN"
              >
              ...
              <tbody>
                {games.map((game) => {
                  ...
                  return (
                    <tr key={game.gameId}>
                      <td>
                        <span className={styles.sportTag}>{game.sport || 'MLB'}</span>
                      </td>
                      <td>{times.et}</td>
                      <td>{times.dub}</td>
                      <td>{times.melb}</td>
        ...
        <div className={styles.mobileResults}>
          {games.length ? (
            ...
            <p className={styles.empty}>
              No games loaded. Select a date range and click Import MLB.
            </p>
          )}
        </div>
        {games.length === 0 && (
