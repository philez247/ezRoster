...
/**
 * Sync NHL scoreboard for a given date (YYYYMMDD).
 ...
 */
export async function syncNhlScoreboard(date, onProgress, opts = {}) {
  const data = await fetchScoreboardViaProxy('nhl', date)
  const games = normalizeEspnScoreboardToGames(data).map((g) => ({ ...g, sport: 'NHL' }))
  if (onProgress) onProgress(date, games.length, 1, 1)
  const fetchedAtIso = new Date().toISOString()
  let added, updated
  if (!opts.skipMerge) {
    const result = mergeGamesIntoMaster(games, 'NHL', fetchedAtIso)
    added = result.added
...
/**
 * Sync NHL scoreboard for a date range.
 ...
 * @returns {Promise<{ games: Game[], fetchedAtIso: string, added?: number, updated?: number }>}
 */
export async function syncNhlScoreboardRange(startDate, endDate, onProgress, opts = {}) {
  ...
  const withSport = allGames.map((g) => ({ ...g, sport: 'NHL' }))
  const fetchedAtIso = new Date().toISOString()
  let added, updated
  if (!opts.skipMerge) {
    const result = mergeGamesIntoMaster(withSport, 'NHL', fetchedAtIso)
    added = result.added
    updated = result.updated
  }
