...
/**
 * Sync MLB scoreboard for a date range.
 ...
 */
export async function syncMlbScoreboardRange(startDate, endDate, onProgress, opts = {}) {
  ...
  const withSport = allGames.map((g) => ({ ...g, sport: 'MLB' }))
  const fetchedAtIso = new Date().toISOString()
  let added, updated
  if (!opts.skipMerge) {
    const result = mergeGamesIntoMaster(withSport, 'MLB', fetchedAtIso)
    added = result.added
